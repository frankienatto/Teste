                ...
                <div>
                    ...
                    <div className="mt-2 p-3 border rounded-lg max-h-64 overflow-y-auto space-y-4">
                        {Object.entries(permissionCategories).map(([key, category]) => {
                             ...
                             const isAllChecked = categoryPermissions.every(p => formData.permissions.includes(p));
                             const isSomeChecked = categoryPermissions.some(p => formData.permissions.includes(p));
                            return (
                                <div key={key}>
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-3 gap-2 pl-4">
                                        {category.permissions.map(perm => (<label key={perm.id} className="flex items-center gap-2 text-sm"><input type="checkbox" checked={formData.permissions.includes(perm.id)} onChange={(e) => handlePermissionChange(perm.id, e.target.checked)} />{perm.label}</label>))}
                                    </div>
                                </div>
                            )
                        })}
